...
class GT_Plugin_Admin {
    ...
    public function action_gt_en_cz_import() {
        $dao = $this->plugin_container->get_cz_en_transtation_dao();
        // drop table records
        $dao->db->drop_table( $dao->db->en_cz_translation_table );

        // create new table
        $dao->db->create_table( $dao->db->en_cz_translation_table );
	...
	public function action_gt_import_one_record() {
        ...
        $czech_word   = esc_sql( filter_input(INPUT_POST, 'czech_word', FILTER_SANITIZE_STRING ) );
        $english_translation = esc_sql ( filter_input( INPUT_POST, 'english_translation', FILTER_SANITIZE_STRING ) );
		...
		} else if ( $czech_word != "" && $english_translation != ""){
            $dao           = $this->plugin_container->get_cz_en_transtation_dao();
            $insert_result = $dao->insert( array( strtoupper( $czech_word ), $english_translation ) );
        } else {
